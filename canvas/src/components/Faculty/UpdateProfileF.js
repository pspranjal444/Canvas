...
class UpdateProfileF extends Component{
    constructor(props){
        super(props);
        this.state={
    ...
    //     Axios.get('http://localhost:3001/getProfileF', {params: {email}})
    //     .then(res=>{
    ...
    componentDidMount(){
        ...
        console.log(this.props.facultyData);
        this.props.facultyData.map(item => {
    ...
    onSubmit(event){
        ...
        Axios.post('http://localhost:3001/updateProfileF', {email, name, password, phone, about, city, country, company, school, hometown, languages, gender}, {headers: {'Authorization': localStorage.getItem('authToken')}})
        .then(res => {
    ...
    onClick(){
        ...
        Axios.post('http://localhost:3001/imageUploadF', data, {email})
        .then(res=>{
...
function mapDispatchToProps(dispatch){
    return bindActionCreators({fetchFaculty: fetchFaculty, setUpdateDetails: setUpdateDetails}, dispatch);
}

export default connect(mapStateToProps, mapDispatchToProps)(UpdateProfileF);